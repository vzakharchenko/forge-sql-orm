...
describe('SQL Hints', () => {
  describe('injectSqlHints', () => {
    ...
    it('should handle multiple hints for different query types', () => {
      ...
      expect(injectSqlHints(selectQuery, hints)).toBe('SELECT /*+ MEMORY_QUOTA(1 GB) */  * FROM users');
      expect(injectSqlHints(insertQuery, hints)).toBe('INSERT /*+ IGNORE_DUP_KEY */  INTO users (name) VALUES ("John")');
      expect(injectSqlHints(updateQuery, hints)).toBe('UPDATE /*+ MAX_EXECUTION_TIME(500) */  users SET name = "John"');
      expect(injectSqlHints(deleteQuery, hints)).toBe('DELETE /*+ MAX_EXECUTION_TIME(200) */  FROM users');
    });
