...
const optimizedQuery = forgeSQL
    ...
    .from(orderItem, {
        forceIndex: [
            ...
            'idx_order_item_created_name_qty'
        ]
    })
    .leftJoin(product, eq(orderItem.productId, product.id))
    .leftJoin(category, eq(product.categoryId, category.id), {
...
resolver.define("deOptimizeQuery", async (): Promise<void> => {
    ...
    await forgeSQLClient.executeDDL('DROP INDEX IF EXISTS idx_order_item_created_name_qty ON order_item');
...
resolver.define('event-listener', async (request) => {
    ...
    for (let i = 0; i < categoryCount; i++) {
        categories.push({
            id: v4(),
            name: `category${i}`,
            createdAt: getRandomDate(new Date('1999-02-12T01:57:45.271Z'), new Date('2025-04-12T01:57:45.271Z'))
        });
    }
...
i=0;
    ...
    console.log(`DATABASE INITIALIZED with orderItems=${orderItems.length}, products=${products.length}, catalogs=${categories.length}`)
});
