...
resolver.define("getUsers", async (req): Promise<{
    users: InferSelectModel<typeof appUser>, organization: InferSelectModel<typeof organization>
}[]> => {
    ...
    if (searchTerm && organizationId) {
        joinQuery.where(and(ilike(appUser.name, searchTerm), eq(appUser.organizationId, organizationId)))
    } else if (searchTerm) {
...
resolver.define("getOrganizations", async (req): Promise<{
    id: number, name: string
}[]> => {
    return forgeSQL.select({id: organization.id, name: organization.name}).from(organization)
});

resolver.define("createUser", async (req): Promise<{
    ...
    return {
        id, name:user.name ?? '', organizationId:user.organizationId??0
    }
});

resolver.define("getUserStats", async (req): Promise<{
    totalUsers: number;
    orgBreakdown: {
        [orgId: number]: number;
    }
}> => {
   const totalUsers = await forgeSQL.fetch().executeQueryOnlyOne(forgeSQL.select({count: sql<number>`COUNT(*)`}).from(appUser));
   const organizations = await forgeSQL.select({organizationId: appUser.organizationId, count: sql<number>`COUNT(*)`}).from(appUser).groupBy(appUser.organizationId);
   const orgBreakdown: {
        [orgId: number]: number;
    } = []
