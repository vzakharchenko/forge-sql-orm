...
export default observer((props: { type: UPDATE_TYPE }) => {
  ...
  const handleSubmit = async (event: any) => {
    ...
    const status:string = await invoke<string>("update", { data: newData, objectName: props.type });
    setStatus(status);
  };
  ...
  return (
          <div>
            ...
            }}>Back to main</button></div>
            <br/>
            <form onSubmit={handleSubmit}>
              <div key="testNameColumn">
                <label htmlFor="testNameColumn">Name</label>
                <br />
                <input type="text" id="testNameColumn" name="testNameColumn" defaultValue={data.testNameColumn} onChange={handleChange} />
                <br />
              </div>
              <div key="testDateColumn">
                <label htmlFor="testDateColumn">dateTime</label>
                <br />
                <input type="datetime-local" id="testDateColumn" name="testDateColumn" defaultValue={String(data.testDateColumn)} onChange={handleChange} />
                <br />
              </div>
              <br/>
            ...
            <br/>
            <br/>
            {status? <div><p>STATUS:</p><b> {status}</b></div>: null}
