...
export default observer(() => {
    ...
    const handleSubmit = async (event:any) => {
        ...
        await invoke('create', {type: 'USER', data: formData});
        usersStore.loading()
        try {
            usersStore.saveUsers(await invoke<DynamicResponse[]>('fetch', {type: 'USER', sortType: usersStore.sort}));
                ...
                {Object.keys(usersMetadata.metadata.user.properties).map((p) => {
                    ...
                    return (
                        <div key={p}>
                            ...
                            <input
                                type={property.type === "integer" || property.type === "smallint" ? "number" : "text"}
                                id={p}
                                name={p}
