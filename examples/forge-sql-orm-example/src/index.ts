...
resolver.define('duplicate', async (): Promise<DuplicateResponse[]> => {
    ...
    const selectFields: Array<string | Knex.Raw> = [...fields, forgeSQL.getKnex().raw('COUNT(*) as count')];

    const query = await forgeSQL.createQueryBuilder(UsersSchema)
    ...
    const duplicateResult = await forgeSQL.fetch().executeSchemaSQL<DuplicateResult>(query, DuplicateSchema);
...
resolver.define('fetch', async (req): Promise<DynamicResponse[]> => {
    ...
    if (sortType?.name) {
        const orderBy: Record<string, QueryOrder> = {
            [sortType.name]: sortType.sortType === 'ASC' ? QueryOrder.ASC_NULLS_FIRST : QueryOrder.DESC_NULLS_LAST
        };
    ...
    return result.map((r): DynamicResponse => ({
        id: r.id,
