...
async function fetchCheckList(issueId: string): Promise<CheckListData | null> {
    const result = await forgeSQL.fetch().executeQueryOnlyOne(
        forgeSQL.select({issueCheckList})
            .from(issueCheckList)
...
resolver.define("updateWithOptimisticLocking", async (request): Promise<{data?: CheckListData, error?: boolean, concurrent?: boolean, message?: string}> => {
...
resolver.define("update", async (request): Promise<{data?: CheckListData, error?: boolean, concurrent?: boolean, message?: string}> => {
    const payload: CheckListData = request.payload as CheckListData;
    if (!payload) {
        return {error: true, concurrent: false, message: 'Payload is empty'};
    }
    ...
    try {
        ...
        return {error: true, concurrent: false, message: 'Update failed'};
    }
});
